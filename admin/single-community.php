	...
	if($post->post_type=='community'&&!is_page()){
		...
		<div class="cmSecA">
		  ...
		  <?php
			...
			<?php if(!empty($cmgallery)){ echo '<ul class="abr-photo-section-block">'; ?>
			
			<?php	foreach($cmgallery as $cmgal){
						echo '<li class="abr-photo-image"><img class="alignleft size-medium" width="165" src="'.$cmgal.'"  data-darkbox="'.$cmgal.'" data-darkbox-group="one"></li>';
					}
				echo '<ul>'; 
				} 
			?>
			...
			<div class="amnt-and-favlink"><?php
				...
				<div class="favlinks">
					...
					<?php
						...
						echo '<li><a href="'.get_permalink($alistid).'?cmid='.$cmid.'" class="arBtn">'.__('Floor Plan and Prices','ar').'</a></li>';
